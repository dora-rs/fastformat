...
impl Image {
    ...
    /// # Arguments
    ///
    /// * `pixels` - A `Vec<u8>` containing the pixel data in RGB8 format.
    ...
    /// let image = Image::new_rgb8(pixels, 3, 3, Some("example")).unwrap();
    /// ```
    ...
    ///
    /// let pixels = vec![0; 27]; // 3x3 image with 3 bytes per pixel
    /// let mut image = Image::new_rgb8(pixels, 3, 3, Some("example")).unwrap();
    ///
